...
本プロジェクト Arclith とそのすべてのモジュールは、作者が勤務時間外に個人的に設計・実装した完全なオリジナル作品です。現在または過去の雇用主から提供されたコード、資料、内部ドキュメント、専有情報は一切含まれていません。
本プロジェクトは、在職中または過去に所属していた企業が使用・展開・開発した商用プロジェクトとは無関係であり、商用環境での利用実績もありません。

本リポジトリの内容は、個人ポートフォリオ、学習・研究、および技術交流 のみを目的としています。商用利用や大規模な引用を希望される場合は、必ず作者までご連絡のうえ、事前に許諾を取得してください。
